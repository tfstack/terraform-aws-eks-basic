...
# Secrets Manager: IAM role for application pods mounting secrets via Secrets Store CSI Driver
# Per-namespace SA and least-privilege policy
...
**Required when using Pod Identity:**

- Add `eks-pod-identity-agent` to `addons`
- Set `secrets_manager_associations` to a list of `{namespace, service_account}` (e.g. `[{namespace = "sm-operator-system", service_account = "awssm-sync"}]`)
- Create each namespace and ServiceAccount in Kubernetes before deploying pods
- Set `usePodIdentity: "true"` in your SecretProviderClass `parameters`

**Per-namespace SA:** One IAM role is shared; one Pod Identity association is created per `secrets_manager_associations` entry. Each namespace can use a different service account (e.g. `awssm-sync` for CSI sync workloads).
...
| [aws_eks_pod_identity_association.sqs](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/eks_pod_identity_association) | resource |
...
| [aws_iam_policy_document.sqs_assume_role_irsa](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
| [aws_iam_policy_document.sqs_assume_role_pod_identity](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| <a name="input_ebs_csi_driver_identity_type"></a> [ebs\_csi\_driver\_identity\_type](#input\_ebs\_csi\_driver\_identity\_type) | Identity type for EBS CSI driver. Use 'pod\_identity' to create Pod Identity association; requires eks-pod-identity-agent addon. | `string` | `"irsa"` | no |
| <a name="input_ebs_csi_driver_namespace"></a> [ebs\_csi\_driver\_namespace](#input\_ebs\_csi\_driver\_namespace) | Kubernetes namespace for EBS CSI driver service account. Used for IRSA OIDC condition and when ebs\_csi\_driver\_identity\_type = 'pod\_identity'. | `string` | `"aws-ebs-csi-driver"` | no |
| <a name="input_ebs_csi_driver_service_account"></a> [ebs\_csi\_driver\_service\_account](#input\_ebs\_csi\_driver\_service\_account) | Kubernetes service account name for EBS CSI driver. Used for IRSA OIDC condition and when ebs\_csi\_driver\_identity\_type = 'pod\_identity'. | `string` | `"ebs-csi-controller-sa"` | no |
...
| <a name="input_enable_secrets_manager"></a> [enable\_secrets\_manager](#input\_enable\_secrets\_manager) | DEPRECATED: use modules/eks-workload-iam instead for per-SA, per-scope grants. Will be removed in a future major version. Whether to create IAM role for Secrets Manager (IRSA or Pod Identity per secrets\_manager\_identity\_type) | `bool` | `false` | no |
...
| <a name="input_private_access_cidrs"></a> [private\_access\_cidrs](#input\_private\_access\_cidrs) | List of CIDR blocks that can access the Amazon EKS private API server endpoint | `list(string)` | `[]` | no |
...
| <a name="input_secrets_manager_associations"></a> [secrets\_manager\_associations](#input\_secrets\_manager\_associations) | DEPRECATED: use modules/eks-workload-iam instead. Will be removed in a future major version. List of {namespace, service\_account} for Pod Identity / IRSA. Each entry gets a Pod Identity association (or IRSA subject). Use app namespaces (e.g. sm-operator-system). Do not use 'default' or 'kube-system'. | <pre>list(object({<br/>    namespace       = string<br/>    service_account = string<br/>  }))</pre> | `[]` | no |
| <a name="input_secrets_manager_enable_parameter_store"></a> [secrets\_manager\_enable\_parameter\_store](#input\_secrets\_manager\_enable\_parameter\_store) | DEPRECATED: use modules/eks-workload-iam with managed\_policy\_arns = [AmazonSSMReadOnlyAccess ARN] instead. Will be removed in a future major version. Whether to also attach AmazonSSMReadOnlyAccess for AWS Systems Manager Parameter Store. Use when application pods need to read parameters in addition to secrets. | `bool` | `false` | no |
| <a name="input_secrets_manager_identity_type"></a> [secrets\_manager\_identity\_type](#input\_secrets\_manager\_identity\_type) | DEPRECATED: use modules/eks-workload-iam instead. Will be removed in a future major version. Identity type for Secrets Manager. Use 'pod\_identity' to create Pod Identity association; requires eks-pod-identity-agent addon. | `string` | `"irsa"` | no |
| <a name="input_secrets_manager_secret_name_prefixes"></a> [secrets\_manager\_secret\_name\_prefixes](#input\_secrets\_manager\_secret\_name\_prefixes) | DEPRECATED: use modules/eks-workload-iam instead. Will be removed in a future major version. List of secret name prefixes (e.g. ['bitwarden/sm-operator']) for least-privilege policy. When non-empty, creates custom policy with GetSecretValue+DescribeSecret instead of AWSSecretsManagerClientReadOnlyAccess. | `list(string)` | `[]` | no |
...
| <a name="input_sqs_access"></a> [sqs\_access](#input\_sqs\_access) | List of SQS access configs. Each entry gets its own IAM role (namespace + service\_account + queue\_arns + mode). Do not use namespace 'default' or 'kube-system'. | <pre>list(object({<br/>    namespace       = string<br/>    service_account = string<br/>    queue_arns      = list(string)<br/>    mode            = optional(string, "consumer") # consumer, read_only<br/>  }))</pre> | `[]` | no |
| <a name="input_sqs_identity_type"></a> [sqs\_identity\_type](#input\_sqs\_identity\_type) | Identity type for SQS access. Use 'pod\_identity' to create Pod Identity associations; requires eks-pod-identity-agent addon. | `string` | `"irsa"` | no |
...
| <a name="output_dynamodb_role_arns"></a> [dynamodb\_role\_arns](#output\_dynamodb\_role\_arns) | Map of IAM role ARNs for DynamoDB access (when enable\_dynamodb\_access), keyed by namespace/service\_account |
